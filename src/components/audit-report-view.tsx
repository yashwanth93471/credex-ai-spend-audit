...
import { EmailCapture } from "@/app/audit/[id]/email-capture";
...
export function AuditReportView({
  ...
  const getToolName = (toolId: string) =>
    CATALOG.tools.find((t) => t.id === toolId)?.name ?? toolId;

  return (
    <div className="min-h-screen bg-zinc-50 py-12 px-4">
      <div className="max-w-4xl mx-auto">

        {/* Persistence degradation notice */}
        {persistenceFailed && (
          <div className="mb-6 px-4 py-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
            Audit generated successfully. Persistence temporarily unavailable.
          </div>
        )}
        ...
        {/* Email capture — only for persisted audits with a real ID */}
        {auditId && (
          <EmailCapture auditId={auditId} isClaimed={isClaimed} />
        )}

        {/* Footer */}
        <div className="mt-8 text-center text-sm text-zinc-500">
          <p>Generated by Credex v{engineVersion}</p>
          <p className="mt-1">
